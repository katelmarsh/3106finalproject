...
for(i in 0:k){
  j <- i*20
  get_url <- paste0("https://api.nytimes.com/svc/movies/v2/reviews/search.json?opening-date=2019-03-015:2021-013-15&api-key=wgJfZeGJBfKXqHXLC7MKYIhusdKGy2y5&offset=",j)
...
for(i in 1:length(rownames(data))){
  text <- get_tokens(data$summary[i])
  syuzhet_vector <- get_sentiment(text, method="syuzhet")
  data$sent_mean[i] <- mean(syuzhet_vector)
  data$sent_sd[i] <- sd(syuzhet_vector)
  data$sent_sum[i] <- sum(syuzhet_vector)
}
tail(data)
...
data[which(data$sent_sum ==max(data$sent_sum)),] # most positive 
data[which(data$sent_sum ==min(data$sent_sum)),] # most negative 
...
merged <- merged[which(merged$startYear > 2009),] 
...
final_merge <- merge(merged, data, by.x="primaryTitle", by.y="title", all.y=TRUE)
...
- only .3416 in the top quartile of IMDB ratings also have a critics pick 
- at the same time, there is a significant correlation between the averageRating and the critics_pick 

```{r}
final_merge <- fread("nyt_and_imdb_data.csv")
...
ols <- lm(critics_pick ~ averageRating, data=final_merge)
summary(ols)
...
# average rating and num votes have significant relationships 

ols <- lm(averageRating ~ runtimeMinutes + numVotes + critics_pick + sent_mean + sent_sd + sent_sum, data=final_merge)
summary(ols)
# runtime, numVotes, critics pick, and sent sd have a significant relationship 
